...
import { get_circuit_breaker_stats } from './email_sender.js';

const RABBITMQ_URL = process.env.RABBITMQ_URL;
const EMAIL_QUEUE = 'email.queue';
const EXCHANGE_NAME = 'notifications.direct';
const HEALTH_PORT = process.env.HEALTH_PORT || 3010;
...
const start_worker = async () => {
  ...
  try {
    ...
    connection = await amqp.connect(RABBITMQ_URL, {
      ...
      clientProperties: {
        connection_name: 'email-service-worker',
      },
    });
    ...
    try {
      ...
      console.log(`✓ Queue ${EMAIL_QUEUE} ready`);
    } catch (error) {
      if (error.code === 406) {
        console.error(`\n✗ Queue ${EMAIL_QUEUE} exists with incompatible configuration.`);
    ...
    channel.prefetch(1);
    console.log(`[*] Waiting for messages in queue: ${EMAIL_QUEUE}. To exit press CTRL+C`);
    ...
    channel.consume(EMAIL_QUEUE, async (msg) => {
      if (msg !== null) {
        ...
        try {
          ...
          // Process the message
          const success = await handle_message(msg);
    ...
    const gracefulShutdown = async (signal) => {
      ...
      console.log('Email service worker shut down');
      process.exit(0);
    };
    ...
    if (error.message.includes('ACCESS_REFUSED')) {
      ...
      console.error('\nPlease create a .env file in the email_service directory with:');
